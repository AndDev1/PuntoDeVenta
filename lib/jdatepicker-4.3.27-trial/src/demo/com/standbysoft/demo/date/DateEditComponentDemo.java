...
public class DateEditComponentDemo extends JPanel {
	...
	private JPanel createDateEditingPanel() {
		...
		editingPanel.add(freeTextHintLabel, new GridBagConstraints(2, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 5, 5, 5), 0, 0));
		editingPanel.add(new JLabel(), new GridBagConstraints(3, 0, 5, 1, 1.0, 0.0, GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL, new Insets(0, 5, 5, 5), 0, 0));
	...
	class MyDateParser extends DefaultDateParser {
		...
		public Date parse(String text, Date reference, DateFormat format) throws DateParserException {
			if (text.toLowerCase().equals("today")) {
				return cal.getTime();
			} else if (text.toLowerCase().equals("none")) {
				return null;
			} else {
